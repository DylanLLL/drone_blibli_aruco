...
PrecisionLand::PrecisionLand(rclcpp::Node& node) : ModeBase(node, kModeName), _node(node)
{
  ...
  // Configure QoS for DDS compatibility
  auto qos_profile =
      rclcpp::QoS(rclcpp::KeepLast(1)).reliable().durability_volatile().history(rclcpp::HistoryPolicy::KeepLast);

  // Create a separate QoS profile for FMU messages
  auto fmu_qos = rclcpp::QoS(rclcpp::KeepLast(1)).best_effort().durability_volatile();

  _target_pose_sub = _node.create_subscription<geometry_msgs::msg::PoseStamped>(
      "/target_pose", qos_profile, std::bind(&PrecisionLand::targetPoseCallback, this, std::placeholders::_1));

  _vehicle_land_detected_sub = _node.create_subscription<px4_msgs::msg::VehicleLandDetected>(
      "/fmu/out/vehicle_land_detected", fmu_qos,
...
void PrecisionLand::targetPoseCallback(const geometry_msgs::msg::PoseStamped::SharedPtr msg)
{
  if (_search_started)
  {
    auto tag = ArucoTag{
      ...
      .orientation = Eigen::Quaterniond(msg->pose.orientation.w, msg->pose.orientation.x, msg->pose.orientation.y,
